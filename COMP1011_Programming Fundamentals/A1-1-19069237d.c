...
void guessGame(void)
{
    ...
    int round = 1; // number of guesses in each round
    int best = 1001; // minimum number of guesses since the program was started
    ...
    do {
        ...
        // check the current score with best score
        if (best > round) {
            best = round;
        }
        ...
        printf("You made %d guess(es) to reach the answer.\n",round);
        printf("Best score: %d\n",best);
        puts("\nWould you like to play again?");
        printf("%s", "Please type ( 1=yes, 2=no )? ");
        scanf("%d", &response);

        // initialize the number of guesses in each round for next round
        round = 1;
