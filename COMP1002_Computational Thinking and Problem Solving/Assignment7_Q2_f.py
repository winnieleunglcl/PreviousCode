...
def solver():
    ...
    setCritical, connection = criticalNodes(G)
    print("\nThe set of critical nodes is", setCritical, "and each has", connection, "connections.")
...
def criticalNodes(G):
    ...
    for i in G.keys():
        if len(G[i]) > max_len:                    #find the maximum number of links in the nodes
            max_len = len(G[i])
        if connection < len(G[i]) < max_len:
           connection = len(G[i])                  #find the number of links of the critical node

    for i in G.keys():
        if len(G[i]) == connection:
            setCritical.add(i)                     #add the critical node(s) in the set
    
    return (setCritical, connection)
